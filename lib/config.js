...
//  One-of-a-kind originals listed in the shop. Prices are shipping-included
//  (no shipping added at checkout). Once one sells it's marked sold and can't
//  be bought again. `image` is a path in /public (e.g. a gallery image).
// ---------------------------------------------------------------------
export const originals = [
  {
    id: "the-lost-duck",
    title: "The Lost Duck",
    image: "/art/the-lost-duck.jpg",
    price: 180, // shipping included
    size: '20" × 20"',
    description: `An original painting of one little duck who wandered off to find the prettiest pond. A one-of-a-kind piece — when it's gone, it's gone! Price includes shipping.`,
  },
];

// ---------------------------------------------------------------------
//  MAP TOTAL + SPREAD
//  The /reviews map always shows this many orders total. Real orders in
//  the system keep their true locations; the rest is topped up using the
//  spread list below (front to back) so the total stays at the target.
//  Set `mapTargetTotal` to 0 to show only real orders.
// ---------------------------------------------------------------------
export const mapTargetTotal = 30;

// Where the "filler" orders go, and roughly how many each. This list sums
// to the target so it can fill an empty map on its own; as real orders come
// in, the tail of this list is trimmed to keep the total at the target.
export const mapSpread = [
  ["CA", 6], ["TX", 3], ["GA", 3], ["NC", 3], ["OH", 2], ["AZ", 2],
  ["OR", 2], ["NY", 2], ["FL", 2], ["WA", 1], ["CO", 1], ["IL", 1],
  ["MA", 1], ["PA", 1], // = 30
];
