...
// Text an arbitrary number (e.g. a customer). Optionally attach an image
// (MMS) via mediaUrl. Uses the same Twilio number as owner texts.
export async function sendSMSTo(to, message, mediaUrl) {
  const sid = process.env.TWILIO_ACCOUNT_SID;
  const token = process.env.TWILIO_AUTH_TOKEN;
  const from = process.env.TWILIO_FROM;

  if (!sid || !token || !from || !to) {
    console.log("SMS to customer (Twilio not configured):", to, message);
    return { ok: true, skipped: true };
  }

  const params = { To: to, From: from, Body: message };
  ...
  if (!res.ok) {
    console.error("Twilio (customer) error:", await res.text());
    return { ok: false };
  }
  return { ok: true };
}

// Which integrations are switched on (so the admin can see at a glance and
// the "send test" buttons can report whether anything actually went out).
export function integrationStatus() {
  return {
    email: !!process.env.RESEND_API_KEY,
    emailFrom: process.env.CONTACT_FROM || "onboarding@resend.dev",
    text: !!(
    ...
    storage: !!(process.env.UPSTASH_REDIS_REST_URL || process.env.KV_REST_API_URL),
  };
}
...
export async function sendEmail({ to, replyTo, subject, text, html }) {
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to,
      // Replies go to Vivian's inbox by default (even when the email is
      // "sent from" a no-reply domain address). Callers can override.
      reply_to: replyTo || site.contactEmail,
      subject,
      text,
      ...(html ? { html } : {}),
    }),
