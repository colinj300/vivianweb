...
import { listCommissions, listAceos, listStickerOrders, listOriginalSales } from "@/lib/store";
...
export async function orderStateCounts() {
  const [commissions, aceos, stickerOrders, originalSales] = await Promise.all([
    listCommissions(),
    listAceos(),
    listStickerOrders(),
    listOriginalSales(),
