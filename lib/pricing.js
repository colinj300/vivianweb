...
// Flat shipping estimate from the destination, piece size, and medium.
export function computeShipping({ sizeLabel, country, mediumId }) {
  const nums = (sizeLabel || "").match(/\d+(\.\d+)?/g);
  const maxInch = nums ? Math.max(...nums.map(Number)) : 0;
  // Small canvas-paper pieces fit in a stamped envelope — free shipping.
  if (mediumId === "canvas-paper" && maxInch && maxInch <= shipping.freeUpToInches) {
    return 0;
  }
...
export function computeEstimate({
  ...
  const ship = computeShipping({ sizeLabel: sizeLabelForShip, country, mediumId });
  ...
  lines.push({
    label:
      ship === 0
        ? "Shipping (free — fits an envelope)"
        : `Shipping (${isDomestic(country) ? "US" : "international"})`,
    amount: ship,
  });
