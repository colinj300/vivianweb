...
// =====================================================================
//  Sticker orders. Stickers are restockable (not one-of-a-kind), so each
//  purchase is logged here (keyed by Stripe session id) with buyer +
//  shipping so Vivian can fulfill them.
// =====================================================================
const STICKER_ORDER_INDEX = "sticker_order_ids";
const stickerOrderKey = (id) => `sticker_order:${id}`;
const memSo =
  globalThis.__stickerOrderMem ||
  (globalThis.__stickerOrderMem = { ids: [], items: new Map() });

export async function saveStickerOrder(record) {
  const exists = useRedis
    ? await redis(["GET", stickerOrderKey(record.id)])
    : memSo.items.get(record.id);
  if (useRedis) {
    await redis(["SET", stickerOrderKey(record.id), JSON.stringify(record)]);
    if (!exists) await redis(["LPUSH", STICKER_ORDER_INDEX, record.id]);
  } else {
    if (!exists) memSo.ids.unshift(record.id);
    memSo.items.set(record.id, record);
  }
  return record;
}

export async function getStickerOrder(id) {
  if (useRedis) {
    const raw = await redis(["GET", stickerOrderKey(id)]);
    return raw ? JSON.parse(raw) : null;
  }
  return memSo.items.get(id) || null;
}

export async function listStickerOrders() {
  if (useRedis) {
    const ids = (await redis(["LRANGE", STICKER_ORDER_INDEX, "0", "-1"])) || [];
    if (ids.length === 0) return [];
    const raw = await redis(["MGET", ...ids.map(stickerOrderKey)]);
    return raw.filter(Boolean).map((r) => JSON.parse(r));
  }
  return memSo.ids.map((id) => memSo.items.get(id)).filter(Boolean);
}
